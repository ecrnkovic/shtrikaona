...
def register(request):
    ...
    if not valid:
        ...
        return dict(
            register_error_message="Korisnik s danim emailom postoji!"
        )
...
def check_parameters_valid(params):
    ...
    for field_key in REGISTER_KEYS:
        if not field_key in params_keys:
            error = u"Neki od parametara potrebnih za registraciju nisu proslijeđeni"
            return (errors, valid)
...
def login(request):
    ...
    if success:
        return HTTPFound(location=came_from)
    message = "Prijava neuspješna, pokušajte ponovo!"
    return dict(
        login_error_message=message,
        url=request.application_url + '/login',
        came_from=came_from
